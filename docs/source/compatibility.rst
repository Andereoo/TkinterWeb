...
Alternatively, you can install Tkhtml from your system package manager if it is avaliable and then add the parameter :attr:`use_prebuilt_tkhtml=False` when creating your :class:`~tkinterweb.HtmlFrame` or :class:`~tkinterweb.HtmlLabel` widget to use the system Tkhtml. Keep in mind that some crash protection features will no longer work.
